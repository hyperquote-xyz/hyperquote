...
makerReceives   = amountIn - feeAmount
takerReceives   = amountOut  (full quoted amount)
```

The fee is denominated in `tokenIn` (the taker's input token) and sent to the fee recipient. The taker receives the full quoted `amountOut`. The maker receives `amountIn - feeAmount`. At `feePips = 250` (2.5 bps / 0.025%), the fee on a 10,000 USDC input is 2.5 USDC.
